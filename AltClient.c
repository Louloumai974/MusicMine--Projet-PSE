...
int main(int argc, char *argv[]){
    ...
    switch(action){
      case 0:
        printf("Rechercher sur MusicMine : ");
        fgets(SongName,LIGNE_MAX,stdin);
        Presence = SearchSong(SongName);

      case 1:
        printf("Ajouter sur MusicMine : ");
        fgets(SongName,LIGNE_MAX,stdin);
        AddSong(SongName);

      case 2:
        printf("Jouer depuis MusicMine :");
        fgets(SongName,LIGNE_MAX,stdin);
        if (SearchSong(SongName) == 0){
    		FILE *fp = fopen(strcat(SongName,".mp3"), "rb");
    		if(fp == NULL){
        	perror("File");
        	return 2;
    }
